...
const OptionButton = ({ opt, optText, state, onClick, disabled }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} className={`w-full text-left p-4 rounded-xl border-2 transition-all duration-200 flex items-start gap-3 group ${styles[state]}`}>
...
const StudentMockExam = () => {
  ...
  const startMode = async (m) => {
    ...
    try {
      ...
      if (questionsList.length === 0) { alert("No questions found."); setIsStarting(false); setLoading(false); return; }
      ...
      setScreen(m);
    } catch { alert("Failed to start exam."); setLoading(false); setIsStarting(false); }
  };
  ...
  const selectPractice = (q, opt) => {
    ...
    setRevealed((p) => ({
      ...p, [q.id]: {
        correct: optionLabel(ci), correctText: (q.options || [])[ci] || "",
        isCorrect: opt === optionLabel(ci), userLetter: opt,
        userText: (q.options || [])[opt.charCodeAt(0) - 65] || "", explanation: q.explanation || "",
      }
    }));
  };
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-10 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-10">
            <div className="inline-flex items-center justify-center w-16 h-16 bg-white dark:bg-gray-800 rounded-2xl shadow-lg mb-4 border border-gray-200 dark:border-gray-700">
              <img src="/logoIcon.png" alt="Exora" className="w-12 h-12 object-contain" />
            </div>
            <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">{course?.name}</h1>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg transition overflow-hidden">
              ...
              <div className="p-7">
                ...
                <ul className="space-y-2.5 mb-8">
                  {["Answer reveals immediately", "Color-coded feedback", "Detailed explanation", "No time pressure"].map((t) => (
  ...
  if (screen === "result" && result) {
    ...
    const incorrect = (result.totalQuestions ?? 0) - correct;
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-10 px-4">
        <div className="max-w-2xl mx-auto space-y-5">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
            <div className={`p-8 text-center ${passed ? "bg-gradient-to-br from-emerald-500 to-teal-600" : "bg-gradient-to-br from-rose-500 to-red-600"}`}>
              ...
              <p className="text-white/80 text-sm mb-5">{course?.name || "Mock Exam"} · {mode === "practice" ? "Practice" : "Test"}</p>
              <div className="inline-block bg-white/15 rounded-2xl px-8 py-4">
                ...
                <p className="text-white/70 text-xs mt-1">{passed ? "✓ Passed (60%)" : "✗ Below 60%"}</p>
              </div>
            ...
            <div className="space-y-4">
              <div className="sticky top-3 z-10 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm px-5 py-3 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-800 dark:text-white">{course?.name || "Mock Exam"}</p>
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                ...
                <div className="px-6 pt-6 pb-2">
                  <p className="text-base font-medium text-gray-800 dark:text-white leading-relaxed mb-6">{currentQ.question_text || currentQ.text}</p>
